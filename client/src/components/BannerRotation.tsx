  ...
  if (banners.length === 0) {
    return (
      <div className={`flex items-center justify-center ${className}`}
        ...
        <a href="/advertise"
          className="flex items-center justify-center w-full h-full border border-dashed border-border/50 rounded-lg bg-muted/10 hover:bg-muted/20 transition-colors cursor-pointer"
          style={{ maxWidth: dims.width, height: dims.height }}
          data-testid={`link-advertise-${zone}`}>
          <div className="text-center">
            <span className="text-xs font-medium text-muted-foreground block">Advertise Here</span>
    ...
    <div className={`flex items-center justify-center ${className}`}
      ...
      <a href={banner.targetUrl} target="_blank" rel="noopener noreferrer sponsored"
        ...
        <img src={banner.imageUrl} alt={banner.name || "Advertisement"}
          style={{ width: dims.width, height: dims.height, maxWidth: "100%", objectFit: "cover" }}
          className="rounded"
          loading="lazy" />
      </a>
