...
- Coin detail panel — tabbed CoinGecko-style layout with 4 tabs: **Overview** (price, 24h range, 7d sparkline, stats, ATH/ATL, sentiment, extended price changes), **Info** (official links, socials, chain explorers, GitHub repos, contract addresses, quick links to CoinGecko/CoinMarketCap/TradingView/Google/X, developer activity, community stats, network info, DeFi ratios, **CoinMarketCap data** — CMC rank, market dominance, market pairs, volume change, 60d/90d changes, CMC tags, additional links/explorers/technical docs from CMC), **Markets** (exchange tickers via `/api/coin/:id/tickers` — exchange logo, pair, USD price, 24h volume, spread, trust score, trade link; top 50 pairs by volume), **News** (filtered articles from CryptoCompare matching coin symbol/name); tab resets to Overview on coin change; Markets/News/CMC queries lazy-loaded
