...
- Crypto Arbitrage Scanner at `/arbitrage` — real-time cross-exchange price comparison for top 20 coins (BTC, ETH, SOL, XRP, BNB, DOGE, ADA, DOT, AVAX, LINK, LTC, TRX, UNI, XLM, NEAR, SUI, APT, ARB, OP, RNDR) + 20 selectable extras (PEPE, SHIB, ATOM, FIL, IMX, INJ, GRT, AAVE, ALGO, FTM, VET, THETA, EOS, MKR, XMR, HBAR, SAND, MANA, AXS, POL); per-coin exchange dedup (best ticker per exchange); spread % with color-coded labels (Excellent/Strong/Moderate/Low/Minimal), visual spread bars; "Best Opportunity" highlight card; profit estimator calculator per coin (gross/fees/net); quick filters (All/Strong 1%+/Moderate/Low), sort by spread/volume/exchanges/name; auto-refresh countdown timer; loading skeletons; expandable exchange price list with trade links; "How Arbitrage Works" guide. Backend: `/api/arbitrage` (90s cache per coin-set, 300ms delay between CoinGecko ticker fetches), `/api/arbitrage/extra-coins`
...
- NFT Explorer at `/nfts` — Alchemy NFT API v3 (free demo key, no API key required) with 5 main tabs: **Top Collections** (5 chains: Ethereum/Polygon/Arbitrum/Optimism/Base, ranked grid cards with floor prices from OpenSea+LooksRare, verified badges, 20 hardcoded top ETH contracts), **Search** (searchContractMetadata across Ethereum), **Wallet NFTs** (address+chain lookup, spam filter, NFT grid with detail modal showing traits, wallet collections overview via getContractsForOwner), **Collection Explorer** (contract address input with 3 sub-tabs: NFTs browser via getNFTsForContract, Stats & Floor via getContractMetadata+getFloorPrice showing deployer/deploy block/token type, Sales History via getNFTSales with price in ETH/seller/buyer/marketplace), **NFT Lookup** (individual NFT by chain/contract/tokenID via getNFTMetadata with full metadata, traits, rarity rank, spam detection, contract deployer, metadata URL). Backend: 11 Alchemy proxy routes (`/api/nfts/collections`, `/api/nfts/search`, `/api/nfts/collection/:address`, `/api/nfts/collection/:address/nfts`, `/api/nfts/collection/:address/stats`, `/api/nfts/wallet/:address`, `/api/nfts/sales/:address`, `/api/nfts/asset/:chain/:contract/:tokenId`, `/api/nfts/account/:address/collections`). Alchemy base URLs: `https://{chain}-mainnet.g.alchemy.com/nft/v3/demo`. Input validation (chain whitelist, EVM address regex), 2min cache with 100-entry limit, `Array.isArray()` guards, admin feature toggle `nfts_enabled`
...
- Blog at `/blog` — full blog system with article listing, category filter, search, pagination, featured posts; single post view at `/blog/:slug` with social sharing (100+ networks via SocialShare modal), view counter, related posts; AI Blog Generator (template-based, free, no API key) in admin Blog tab with topic/category/tone inputs + quick-pick suggestions; Auto-Fill SEO button populates metaTitle, metaDescription, metaKeywords from content; Google Search Preview (SERP preview) shows how blog post appears in Google
- SEO Manager — admin SEO tab with 17 pages, per-page auto-fill from optimized templates (POST /api/admin/seo/auto-fill), "Auto-Fill All Pages" bulk action, Google SERP preview per page, SEO score checker (title/desc length, keywords, OG tags)
...
- Admin panel at `/admin` with:
  ...
  - **Banners tab** — full banner ad management system with 4 sub-tabs: **Manage Banners** (CRUD for ad banners with name/imageUrl/targetUrl/size/zone/active/sortOrder, click stats, toggle active/inactive, edit/delete), **Inquiries** (view advertiser inquiry messages from `/advertise` page, reply to inquiries, change status pending/replied/closed), **Settings** (banner_rotation_enabled, banner_rotation_interval, banner_footer_enabled, banner_blog_enabled toggles), **Code Generator** (affiliate embed code generator with 7 sizes, presets, color pickers, animation, copy HTML). Banner sizes: 728x90, 468x60, 300x250, 160x600, 320x50, 970x90, 336x280. Zones: header, footer, sidebar, blog-top, blog-middle, blog-bottom
  - **Settings tab reorganized into sub-tabs**: General (home page, site title, password, login link, affiliate ID), AdSense (publisher ID, slot IDs, ads.txt), Maintenance (global maintenance mode toggle, custom message, allowed IPs, scheduled maintenance info), Features (14 toggle switches for wallet tracking, prices, news, masternodes, swap, blog, chat, airdrops, arbitrage, calculator, staking, DEX, gold, contact), API Keys (Etherscan, CMC, ChangeNOW, Brave — stored in DB for server migration)
  - **Maintenance Mode** — server-side middleware returns 503 maintenance page to all public visitors when enabled; admin panel always accessible; custom message support; IP allowlist bypass; styled maintenance page matching cyberpunk theme; API endpoints return JSON 503
  - **Feature Toggles (Frontend)** — `FeatureContext` in `App.tsx` provides settings from `/api/site-settings` to all components; `useFeatureEnabled(key)` hook checks if feature is enabled (defaults `true` when key missing); `FeatureRoute` wraps routes to show "Feature Unavailable" page when disabled; `Navbar.tsx` filters nav links based on feature settings; home page falls back to Explorer if selected home page feature is disabled
  - **Spam Monitor tab** — spam bot detection engine monitors all chat messages for: scam keywords (30+ patterns), URL spam, flood detection (15+ msgs/5min), duplicate messages (3+ repeats), repeated chars, ALL CAPS; severity levels (low/medium/high/critical); auto-bans IP after 5 spam reports (24h block); `spam_reports` DB table; admin dashboard with stats cards (total/unresolved/auto-banned/offenders), top offenders list with one-click ban, manual IP ban with duration selector, spam reports feed with filter/resolve/bulk-resolve/delete; API routes under `/api/admin/spam/*`
- Historical Snapshot at `/snapshot` — view market data from any date in the past year, compare with today's prices. Uses CoinGecko `/coins/:id/market_chart` endpoint with retry logic (429/5xx + network error handling, exponential backoff with jitter). Top 5 coins (BTC, ETH, XRP, SOL, DOGE), 6s delay between calls, 24h cache. 365-day limit enforced (CoinGecko free tier). Empty state shown when rate-limited. UTC date boundary calculation.
- Portfolio Performance Graph on `/portfolio` — Recharts area chart showing portfolio value over time via localStorage daily snapshots. Records snapshot on each page load with live prices. Timeframe toggles: 7d/30d/All. Shows gain/loss trend with color-coded areas.
- Beginner's Guide at `/learn` — educational accordion page with 9 sections: What is Cryptocurrency, How Blockchain Works, Types of Crypto, How to Buy Crypto, Wallet Types, Key Terms Glossary, Safety Tips, DeFi Basics, NFT Basics. Visual cards with icons.
...
- Banner Ad Rotation System — `BannerRotation` component (`client/src/components/BannerRotation.tsx`) fetches active banners per zone from `/api/banners/:zone`, auto-rotates on configurable interval, click tracking with IP-based rate limiting (30s per IP per banner), "Advertise Here" placeholder with link to `/advertise` when no banners exist. Zones: header, footer, sidebar, blog-top, blog-middle, blog-bottom. Footer shows 468x60 banner (controlled by `banner_footer_enabled`), blog page shows banners at top/middle/bottom (controlled by `banner_blog_enabled`). Advertiser inquiry form at `/advertise` submits to `banner_inquiries` table. DB tables: `banners` (id, name, imageUrl, targetUrl, size, zone, clicks, active, sortOrder, createdAt), `banner_inquiries` (id, name, email, company, bannerSize, message, status, adminReply, createdAt)
