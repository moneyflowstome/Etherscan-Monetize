...
- NFT Explorer at `/nfts` — Full OpenSea API v2 integration with 5 main tabs: **Trending Collections** (7 chains, ranked grid cards with floor/volume/owners/supply, verified badges), **Search** (multi-chain collection search), **Wallet** (address+chain lookup, NFT grid with detail modal, wallet collections overview), **Collection Explorer** (slug lookup with 6 sub-tabs: NFTs browser, Stats with interval data, Activity feed with event type filters for sales/listings/transfers/offers/cancellations, active Listings with prices, active Offers, Traits breakdown with expandable categories), **NFT Lookup** (individual NFT by chain/contract/tokenID with full metadata, traits, rarity, creator, owner, links). Backend: 12 proxy routes (`/api/nfts/collections`, `/api/nfts/search`, `/api/nfts/collection/:slug`, `/api/nfts/collection/:slug/nfts`, `/api/nfts/collection/:slug/stats`, `/api/nfts/wallet/:address`, `/api/nfts/events`, `/api/nfts/listings/:slug`, `/api/nfts/offers/:slug`, `/api/nfts/traits/:slug`, `/api/nfts/asset/:chain/:contract/:tokenId`, `/api/nfts/account/:address/collections`). Input validation (chain whitelist, EVM address regex, slug sanitization), proper HTTP status mapping (400/404/429/502), 2min cache, `Array.isArray()` guards, admin feature toggle `nfts_enabled`
