...
- Admin panel at `/admin` with:
  ...
  - **API Keys section** in Settings tab — store API keys (Etherscan, CMC, ChangeNOW, Brave) in the database so they transfer with the DB when migrating servers; keys are masked in the API response; overrides environment variables; keys refresh on save
