...
- NFT Explorer at `/nfts` — OpenSea API v2 integration with 4 tabs: **Trending Collections** (chain filter for Ethereum/Polygon/Arbitrum/Optimism/Base/Avalanche/BSC, ranked grid cards with floor price/volume/owners/supply stats, verified badges), **Search** (collection name search with card results), **Wallet NFTs** (address + chain lookup, NFT grid with detail modal showing traits/description/links), **Collection Detail** (slug lookup with stats panel, expandable NFT browser). Backend proxy routes: `/api/nfts/collections`, `/api/nfts/search`, `/api/nfts/collection/:slug`, `/api/nfts/collection/:slug/nfts`, `/api/nfts/wallet/:address`. 2min cache, `Array.isArray()` guards, admin feature toggle `nfts_enabled`
